...
## 59. Intro to Network Automation
...
- `Network automation` provides many benefits:
  ...
  - The The improved efficiency of network operations reduces the OpEx of the network.
...
- A router recieves a messages, looks for the most specific matching route in its routing table, and forwards it out of the appropriate interface to the next hop.
...
- Functions that build tables like routing table, MAC table, ARP table, STP, etc. (and other functions that influence the data plane) are part of the `control plane`.
- The control plane controls what the data plane does, for example by building the router's router table.
...
- In traditional networking, the data and control plane are both distributed. Each device has its own data and control plane.

#### Management Plane

- Like control plane, `management plane` performs overhead work.
...
- The management plane consists of protocols that are used to manage devices:
  - SSH/Telnet, used to connect to the CLI of device to configure it
...
- Instead, a specialized hardware ASIC is used. ASICs are chips built for specific purposes.
- Using switch as an example:
  - When a frame is received, the ASIC is responsible for the switching logic
  - The MAC table is stored in a kind of memory called `TCAM (Ternary Content-Addressable Memory)`
